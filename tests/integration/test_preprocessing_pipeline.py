...
class TestPreprocessingPipelineIntegration:
    ...
    def test_full_preprocessing_pipeline(self, fastrs_with_sample_data):
        ...
        result = fastrs_with_sample_data.preprocess(option="default")
        ...
        assert all(isinstance(sentence, list) for sentence in fastrs_with_sample_data.feed)
        assert all(isinstance(token, str) for sentence in fastrs_with_sample_data.feed for token in sentence)
...
class TestPreprocessingWithRealData:
    ...
    def test_literature_question_preprocessing(self, literature_fastrs):
        ...
        # Run preprocessing
        result = literature_fastrs.preprocess(option="default")
        ...
        feed_tokens = [token for sentence in literature_fastrs.feed for token in sentence]
    ...
    def test_science_question_preprocessing(self, science_fastrs):
        ...
        # Run preprocessing
        result = science_fastrs.preprocess(option="default")
    ...
    def test_mixed_subject_preprocessing(self, mixed_subjects_fastrs):
        ...
        # Run preprocessing
        result = mixed_subjects_fastrs.preprocess(option="default")
...
class TestPreprocessingPerformance:
    ...
    def test_preprocessing_scalability(self, large_dataset_fastrs):
        ...
        result = large_dataset_fastrs.preprocess(option="default")
        end_time = time.time()
...
def literature_fastrs():
    ...
    filtered_data = {k: v for k, v in literature_data.items() if "문학" in k or "시" in v.get("information", "")}
    if not filtered_data:
        filtered_data = {"item1": literature_data["item1"]}  # Use first item as fallback
...
def science_fastrs():
    ...
    filtered_data = {k: v for k, v in science_data.items() if "과학" in k or "지구" in v.get("information", "")}
    if not filtered_data:
